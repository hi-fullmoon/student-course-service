...
from app.models import CourseModel, CourseScheduleModel, StudentCourseModel
...
async def get_course_schedule(course_id: int, db: Session = Depends(get_db)):
    schedules = (
        db.query(CourseScheduleModel)
...
async def update_course_schedule(
    course_id: int, schedule: CourseScheduleCreate, db: Session = Depends(get_db)
):
    # 验证课程是否存在
    ...
    try:
        ...
        return response_success(message="课程时间安排更新成功", data=response_data)
    except Exception as e:
        db.rollback()
...
async def delete_course_schedule(course_id: int, db: Session = Depends(get_db)):
    # 验证课程是否存在
    course = db.query(CourseModel).filter(CourseModel.id == course_id).first()
    if not course:
        return response_error(code=404, message="课程不存在")
